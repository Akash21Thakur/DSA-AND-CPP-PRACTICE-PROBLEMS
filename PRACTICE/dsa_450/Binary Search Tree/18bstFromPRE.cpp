...
Node *helper(int *pre, int &idx, int key, int mini, int maxi, int size)
{
    if (idx == size)
        return NULL;
    ...
    if (key > mini and key < maxi)
    {
        ...
        if (idx < size)
            root->left = helper(pre, idx, pre[idx], mini, key, size);
        if (idx < size)
            root->right = helper(pre, idx, pre[idx], key, maxi, size);
    }
    return root;
}

// simple tree from array
Node *post_order(int pre[], int size)
{
    int idx = 0;
    return helper(pre, idx, pre[0], INT_MIN, INT_MAX, size);
}
